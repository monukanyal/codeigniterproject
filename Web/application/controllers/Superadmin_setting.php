...
class Superadmin_Setting extends CI_Controller {
    ...
    function __construct() 
    {
        ...
        $this->load->model('user_smodel');
        $this->load->model('organization_model');
        $this->load->model('super_organization_model');
    ...
    public function index($userId)
    {
        ...
        $data['superadmin_info'] = $this->user_smodel->get_admindata($data);
        // echo "<pre>";
        // print_r($data);
        // echo "</pre>";
        //print_r($userId);
        ...
        $this->load->view('super_dashboard/setting/index',$data);
    ...
    function edit($userId,$start=0)
    {
        ...
        $cur_controller=$this->router->fetch_class(); //current controller
        ...
        $edit_data=$this->super_organization_model->get_organization_data(array("id" => $userId));
...
// die();
            ...
            if($this->form_validation->run() == TRUE)               
            {
                ...
                $id_record=$this->super_organization_model->update($data_feed, $default_data, 'ci_super_admin');
                $this->session->set_flashdata('flash_message', 'Record has been updated successfully');
                 redirect(site_url('superadmin_profile'));
            } 
            else 
        ...
        $this->load->view('includes/partials/super_admin/sidebar',$data);
        $this->load->view('includes/partials/super_admin/topmenu',$data);
        $this->load->view('super_dashboard/setting/add',$form_data);
        $this->load->view('includes/partials/footer',$data);
