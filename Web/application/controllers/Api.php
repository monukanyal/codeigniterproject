...
class API extends CI_Controller {	
	...
	*********************************************************************************/	
	...
	function userLogin($data){
		...
		$requiredData['userLogin'] = $data['user'];
		...
		// if( $requiredData['userLogin'] == 'resident')
		// {
			$where['mobile'] = $requiredData['mobilenum'];
			$loggedInUser = $requiredData['userLogin'];
		// }
		// else{
		// 	$where['child_mobile'] = $requiredData['mobilenum'];
		// 	$loggedInUser = $requiredData['userLogin'];
		// }
		...
		if(!empty($arrUser)){
			...
			if( $requiredData['userLogin'] == 'resident'){
				$response = array('user_id'=>$arrUser[0]['id'],'org_id'=>$arrUser[0]['admin_id'],'username'=>$arrUser[0]['first_name']." ".$arrUser[0]['last_name'],'mobile'=>$requiredData['mobilenum'],'status'=>'Success','message'=>'User Login Successfully' ,'registration_status'=>$arrUser[0]['reg_status'],'user_type'=>$arrUser[0]['user_type'], 'user' => $loggedInUser );						
				return $response;
			}
			else
			{
				$resident_info=$this->api_model->get_resident_info($arrUser[0]['belongs_to']);
							if(!empty($resident_info))
							{
								
								$response = array('care_id'=>$arrUser[0]['id'],'user_id'=>$arrUser[0]['belongs_to'],'org_id'=>$arrUser[0]['admin_id'],'username'=>$arrUser[0]['first_name']." ".$arrUser[0]['last_name'],'mobile'=>$requiredData['mobilenum'],'status'=>'Success','message'=>'User Login Successfully' ,'registration_status'=>$arrUser[0]['reg_status'],'user_type'=>$arrUser[0]['user_type'],'resident_mobile'=>$resident_info['mobile'],'user' => $loggedInUser );	
								return $response;
							}
							else
							{
								$response = array('care_id'=>$arrUser[0]['id'],'user_id'=>$arrUser[0]['belongs_to'],'org_id'=>$arrUser[0]['admin_id'],'username'=>$arrUser[0]['first_name']." ".$arrUser[0]['last_name'],'mobile'=>$requiredData['mobilenum'],'status'=>'Success','message'=>'User Login Successfully' ,'registration_status'=>$arrUser[0]['reg_status'],'user_type'=>$arrUser[0]['user_type'],'user' => $loggedInUser );	
								return $response;
							}
	...
	//------------mk-----------------------
		function verify_user($data)
		{
			if(!empty($data['uuid']))
			{
				//$code= @trim($data['uuid']);
				$code= $data['uuid'];
			  $response=$this->api_model->usercheck($code);
			if(!empty($response))
			{
				if($response['user_type']=='resident')
				{
					$result= array('user_id'=>$response['id'],'org_id'=>$response['admin_id'],'username'=>$response['first_name']." ".$response['last_name'],'mobile'=>$response['mobile'],'status'=>'Success','message'=>'User Login Successfully' ,'registration_status'=>$response['reg_status'],'user_type'=>$response['user_type'], 'user' =>'resident');
					return $result;
				}
				else
				{
					if(!empty($response['belongs_to']))
					{
						$resident_info=$this->api_model->get_resident_info($response['belongs_to']);
							if(!empty($resident_info))
							{
								
								$result = array('care_id'=>$response['id'],'user_id'=>$response['belongs_to'],'org_id'=>$response['admin_id'],'username'=>$response['first_name']." ".$response['last_name'],'mobile'=>$response['mobile'],'status'=>'Success','message'=>'User Login Successfully' ,'registration_status'=>$response['reg_status'],'user_type'=>$response['user_type'],'resident_mobile'=>$resident_info['mobile'],'user' => 'child' );	
								return $result;
							}
							else
							{
								$result= array('care_id'=>$response['id'],'user_id'=>$response['belongs_to'],'org_id'=>$response['admin_id'],'username'=>$response['first_name']." ".$response['last_name'],'mobile'=>$response['mobilenum'],'status'=>'Success','message'=>'User Login Successfully' ,'registration_status'=>$response['reg_status'],'user_type'=>$response['user_type'],'user' => 'child' );	
								return $result;
							}
					}
					else
...
//@mkcode
	function list_event_bysitecode($info)
	{
		
		$requiredData['sitecode'] = @trim($info['sitecode']);
		$sitecode = @trim($info['sitecode']);
		if(trim($requiredData['sitecode']) == ''){
			$response[] = array('status'=>'Error','message'=>'Please Specify site code');
			return $response;
		}	
		$data['result'] = $this->user_model->get_admin_bysitecode($sitecode);
			if($data['result']['sitecode_exist'] == "true")
			{
		 		$admin_id = $data['result']['admin_id'];
					...
					$response[] = array('status'=>'Error','message'=>'Sitecode not exists.');
					return $response;
			}
        	...
        	foreach($arrEvents as $arrevent)
			{
            	...
            	//print_r($event_daterange);
					...
					foreach($event_daterange as $event_dates)
		        	{
		        		...
		        		//echo $r_dates.'-';
	        		 	//if($r_dates >= $today)   //<--old
	        		 	if(($r_dates>=$today )||($r_dates<$today))
						{
							//print_r($arrevent);
		                    ...
		                    {
		                    	if (array_key_exists($meetup_date, $response)) 
		                    	{
									//echo "c1yes $r_dates<br>";
									if(array_key_exists($meetup_time, $response[$r_dates]))
									{
										//echo "c1yes $meetup_time<br>";
										if(!in_array($id, $response[$r_dates][$meetup_time][0]))
										{
											//echo "c1no $id<br>";
											$response[$r_dates][$meetup_time][] = $arrevent;
										}
		                    ...
		                    if ( ($r_day == $recring_val) ) 
		                    {
		                    	if (array_key_exists($meetup_date, $response)) 
		                    	{
									//echo "c2yes $r_dates<br>";
									if(array_key_exists($meetup_time, $response[$r_dates]))
									{
										//echo "c2yes $meetup_time<br>";	
										if(!in_array($id, $response[$r_dates][$meetup_time][0]))
										{
											//echo "c2no $id<br>";
											$response[$r_dates][$meetup_time][] = $arrevent;
	...
	*************************************/
	...
	private function send_users_sms($data)
    {
        ...
        $resendsms_data = array(
            'src' => '+18888209850', // +18888209850 The phone number to use as the caller id (with the country code). E.g. For USA 15671234567
    ...
    private function send_activitysms($data)
    {
        ...
        // $site_url = base_url('index.php/applink');
        $sms_data = array(
            'src'    => '+18888209850', //The phone number to use as the caller id (with the country code). E.g. For USA 15671234567
            // 'src'    => '+917696568274',
	...
	{			
		...
		if(!empty($arrEvents))
        {
	        		 	...
	        		 	if($r_dates >= $today) 
						{
		                    ...
		                    if ($r_dates == $meetup_date) 
		                    {
		                    	...
		                    	if (array_key_exists($meetup_date, $response)) 
		                    	{
									...
									{
										//echo "c1yes $meetup_time<br>";
										//echo "counting timearr: ".count($response[$r_dates][$meetup_time]);
			...
			if (empty($response)) {
				...
				$responseArrayObject->ksort();
				//print_r($responseArrayObject);
				//echo "count: ".count($responseArrayObject);
				//@mkcode
				if(count($responseArrayObject)>0)
				{
					foreach ($responseArrayObject as $key => &$value) {
							$range_date=$key;
							//echo $key.'<br>';
							$ev_arry=array();
						foreach($value as $time_key =>&$value2)
						{
							//print_r($value2);
							//echo " count inside : ".count($value2);
							if(count($value2)>=1)
							{
								for($p=0;$p<count($value2);$p++)
								{
									$ev_arry[]=$value2[$p]['id'];
									$evid=$value2[$p]['id'];
									//echo "<br>".$evid."<br>";
									$mkarr=$this->api_model->getstatus_event($evid,$user_id,$range_date);
									if(!empty($mkarr))
									{
										//print_r($mkarr);
										$value2[$p]['status']=$mkarr[0]['status'];  //@mkcode

									}
								}
								$value2 = array_map('unserialize', array_unique(array_map('serialize', $value2)));
							}
					...
					//----new changes start :show next 30days events only--------------------------
					foreach ($responseArrayObject as $key => &$value)
					{
			 				$nextdate=date('Y-m-d', strtotime("+30 days"));
		 				 	if(date($key)>$nextdate)
		 				 	{
		 				 		//echo $key.'\n';
								unset($responseArrayObject[$key]);
							}
					}
					//------new changes end--------------------------------------------------------
					return array($responseArrayObject);	
	...
	*************************************/
	...
	{			
		...
		$admin_id = $requiredData['org_id'];
		$where['admin_id'] = $requiredData['org_id'];
		 //$meals_withdate=$this->api_model->get_calender_meals_withdate($where,'ci_plan_meal');
		$query=$this->api_model->getAll_new($where_like='', $where, $limit='', $offset=0, $orderby='','ci_plan_meal'); // collecting category list
			if (empty($query)) 
			{
					$response[]	= array('status'=>'Error','message'=>'No Record Found.');			
					return $response;
			...
			foreach($query as $key=>$meal)
			{
				//echo "<br>".$meal['end_date'];
				$recurring_m = $meal['recurring'];
				$recurring_days = explode(",", $recurring_m);
				//------meal show restriction------
				$today=date('Y-m-d');
				$today_time = strtotime($today);
				if(!empty($meal['end_date']))
				{
						$expire_time = strtotime($meal['end_date']);
						$start_time = strtotime($meal['start_date']);
				...
				foreach ($recurring_days as $key => $r_value) {	
						...
						if($cur_day==$db_days[$r_value])
						{
							$range_date=$cur_date;
						}
						else
						{
							$range_date=date('Y-m-d', strtotime("next $day"));
						}
						//echo "\nMealid".$mealid;
						//echo "RangeDate:".$range_date;
						$mkarr=$this->api_model->getstatus_meal($mealid,$user_id,$range_date);
						//echo $mkarr[0]['id'].'\n';
						if(!empty($mkarr))
						{
							//print_r($mkarr);
							$status=$mkarr[0]['status'];  //@mkcode
							$location_id = $meal['location_id'];
        					$location_name=$this->api_model->getByID($location_id, $where ='','ci_location'); // collecting category list
					
							$response[$db_days[$r_value]][]= array("id"=>$meal["id"], "name"=>$meal['name'], "meal_type"=>$meal['meal_type'], "description"=>$meal['description'], "location_id"=>$location_id, "location_name"=>$location_name['name'], "start_date"=>$meal['start_date'], "start_time"=>$meal['start_time'], "end_date"=>$meal['end_date'], "end_time"=>$meal['end_time'], "recurring"=>$meal['recurring'],"status"=>$status);
			...
			if(!empty($response['Sunday']))
			{
				$newresponse['Sunday']=$response['Sunday'];	
			}
			// else
			// {
			// 	$newresponse['Sunday']=array();	
			// }
			if(!empty($response['Monday']))
			{
				$newresponse['Monday']=$response['Monday'];	
			}
			if(!empty($response['Tuesday']))
			{
				$newresponse['Tuesday']=$response['Tuesday'];	
			}
			if(!empty($response['Wednesday']))
			{
				$newresponse['Wednesday']=$response['Wednesday'];	
			}
			if(!empty($response['Thursday']))
			{
				$newresponse['Thursday']=$response['Thursday'];	
			}
			if(!empty($response['Friday']))
			{
				$newresponse['Friday']=$response['Friday'];	
			}
			if(!empty($response['Saturday']))
			{
				$newresponse['Saturday']=$response['Saturday'];	
			}
			// if(!empty($response['']))
			// {
			...
			return array($newresponse);	  //new
			//return array($response);	//old
	}
	...
	*************************************/
	...
	{			
		...
		$requiredData['id'] = @trim($data['id']);
		$requiredData['org_id'] = @trim($data['org_id']);
		$requiredData['resident_id'] = @trim($data['resident_id']);  //@MKCODE
		$resident_id = $requiredData['resident_id'];
		foreach($requiredData AS $key=>$val){		
			if(trim($val) == ''){
				$response[]	= array('status'=>'Error','message'=>'Please Specify '.ucwords(str_replace("_"," ",$key)));		
				return $response;
		...
		$this->api_model->remove_oldjoined($mealid,$resident_id);  //@MKCODE	
		$response = array();
		$admin_id = $requiredData['org_id'];
		$where['admin_id'] = $requiredData['org_id'];
	...
	*************************************/
	...
	//@mkcode
	/*function new_listUserdays($data)
	{			
		$requiredData['user_id'] = @trim($data['user_id']);
		...
		if(!empty($myday))
        {
        	if (!empty($events)) 
        	{
        		foreach ($events as $value) 
	        	{
	        		...
	        		$event_time = $value['event_time'];
				 	...
				 	if ($r_dates >= $today ) 
					{        			
	        			$response1[$value['range_date']][$event_time][] =$value; 
			...
			foreach ($meals as $key1 => $meals1) {
        		...
        		$response2[$meals1['range_date']][$meals1['event_time']][] = $meals1;
...
function new_listUserdays($data)
	{			
		...
		$events = $this->api_model->get_my_events_new($user_id);
		$meals = $this->api_model->get_my_meals_new($user_id); 
		...
		if(!empty($myday))
        {
			...
			foreach ($meals as $key1 => $meals1) 
			{
					...
					$w_day = $meals1['week_day'].'-'.$meals1['name'].'-'.$meals1['event_time'];
					
				if (!in_array($w_day, $weeksdays)) 
				{
					
					$weeksdays[] = $meals1['week_day'].'-'.$meals1['name'].'-'.$meals1['event_time'];
	...
	function getAppUser($data){	
			...
			if(!empty($result))
			{
				...
				$response['message'] = 'Congrats!! Your app is successfully activated';
				$response['user_id'] = $result['id'];
				$response['parent_id'] = $result['belongs_to'];
  ...
  //   	}
		...
		if(trim($requiredData['plan_meal_id']) == '' || trim($requiredData['plan_meal_id']) == 0){
			...
			if (empty($arrMeal)) {
			 ...
			 $end_time = new DateTime($e);
			  $time_interval1 = $startTime->format('H:i:s');
			  $time_interval2 = $end_time->format('H:i:s');
			  $response[] = array('time'=>$time_interval1);
			  $response[] = array('time'=>$time_interval2);
	...
	*************************************/
	...
	{			
		$requiredData['id'] = @trim($data['id']);
		$requiredData['user_id'] = @trim($data['user_id']);
		$requiredData['week_day'] = @trim($data['week_day']);
        ...
        $where = array(
			...
			'user_id' => $requiredData['user_id'],
			'week_day'=> $requiredData['week_day']	
		);
        $meal_name=$this->api_model->getrangeByID($where,'ci_plan_mealmeta');
	...
	//@mkcode start --listevent refresh
	private function event_reloader($data)
	{
		$admin_id=$data['admin_id'];
		if(isset($admin_id))
		{
			$numrow=$this->api_model->get_event_or_meal_reloader_info($admin_id,'ci_event_reloader');
	     	if(!empty($numrow)) 
	     	{
	           return array('status'=>'success','numrow'=>$numrow);
	           
	        }
	        else
	        {
	        	return array('status'=>'error','numrow'=>0);
	        }
		...
		else
		{
			...
			return array('text'=>'Please provide required data');
		}
	}
	// --list meal refresh
	private function meal_reloader($data)
	{
		$admin_id=$data['admin_id'];
		if(isset($admin_id))
		{
			$numrow=$this->api_model->get_event_or_meal_reloader_info($admin_id,'ci_meal_reloader');
	     	if(!empty($numrow)) 
	     	{
	            return array('status'=>'success','numrow'=>$numrow);
	...
	//@mkcode 10 may start
	private function care_belongs_to($info)
	{
		$care_id = @trim($info['care_id']);
		if(!empty($care_id))
		{
			$data=$this->api_model->get_care_belongs_to($care_id);
			if(!empty($data))
			{
			...
			{
				return array('status'=>'error','message'=>'Something is wrong,please try again later');
			}
		}
		else
		{
			return array('status'=>'error','message'=>'No care id found');
		}
	}

	

	private function list_all_admin($info)
	{
			$data=$this->api_model->get_alladmin_list();
			if(!empty($data))
			{
	...
	private function list_resident_admin($info)
	{
		$admin_id=@trim($info['admin_id']);
		if(!empty($admin_id))
		{
			$data=$this->api_model->get_resident_list($admin_id);
			if(!empty($data))
			{
		...
		{
			return array('status'=>'error','message'=>'No admin id found');
		}
	}

// first care add
	public function update_child_account($info)
	{
		//  $first_name='aajad';
		//  $last_name='singh';
		//  $mobile='6767676767';
		// $email='aajad@gmail.com';
		// $gender='MALE';
		//   $resident_mob='5125374728';
		...
		$resident_mob=@trim($info['resident_mobile']);

		if(!empty($first_name) && !empty($last_name) && !empty($mobile) && !empty($email) && !empty($gender) && !empty($resident_mob))
		{
				...
				$resident_id=$this->api_model->get_resident_id($resident_mob);
				if(!empty($resident_id))
				{
					$limit=$this->api_model->check_care_account_limit($resident_id);
					if($limit<=4)
					{
						$admin_id=$this->api_model->get_admin_id_ofresident($resident_id);
						$dataq['admin_info']=$this->api_model->get_admindata(array("admin_id" => $admin_id));
						if(!empty($admin_id))
						{
							$property_name=$dataq['admin_info'][0]['address'];
								$complete_data=array(
								...
								$careid=$this->api_model->update_child($complete_data,$mobile);
								if(!empty($careid))
								{
									
									return array('status'=>'success','message'=>'Your profile has been updated successfully.',"care_id"=>$careid,"admin_id"=>$admin_id,"resident_id"=>$resident_id);
								}
								else
								{
									return array('status'=>'error','message'=>'Something is wrong,please try again later');
						...
						{
							return array('status'=>'error','message'=>'Unable to get admin id');
						}

				}
				else
				{
					return array('status'=>'error','message'=>'Care limit exceeds 4');
				}
			}
			else
			{
				return array('status'=>'error','message'=>'Resident Detail Not Found!!');
			}	

		}
		else
		{
			return array('status'=>'error','message'=>'Please provide required data!!');
		}
	}
	private function create_child_account($info)
	{
					...
					if($limit<4)
					{
						...
						if(!empty($admin_id))
						{
							$property_name=$dataq['admin_info'][0]['address'];
								...
								$insert_id=$this->api_model->insert_child($complete_data);
								if(!empty($insert_id))
								{
									
									return array('status'=>'success','message'=>'Care successfully registered',"care_id"=>$insert_id,"admin_id"=>$admin_id,"resident_id"=>$resident_id);
								}
								else
								{
									return array('status'=>'error','message'=>'Something is wrong,please try again later');
	...
	private function care_approval_check($info)
	{	//$care_id='483';
		$care_id=@trim($info['care_id']);
		if(!empty($care_id))
		{
			$status=$this->api_model->get_care_approval_check($care_id);
			if(!empty($status))
			{
				...
				if($status['approve']==1)
				{
					...
					return array('status'=>'success','message'=>'Care Approved.','response'=>$status['approve'],'registration_status'=>$reg_status);	
				}
				else
				{
					return array('status'=>'success','message'=>'Care needs approval by Admin.','response'=>$status['approve'],'registration_status'=>$reg_status);	
				}
			}
			else
			{
				return array('status'=>'error','message'=>'Care is not exist');	
			}
		}
		else
		{
	...
	private function getsitecode_mob($info)
	{
		$mobilenum=@trim($info['mobilenum']);
		if(!empty($mobilenum))
		{
			$response=$this->api_model->getsitecode_mob($mobilenum);
			if(!empty($response))
			{

				return array('status'=>'success','message'=>'sitecode found','response'=>$response);	
			}
			else
			{
				return array('status'=>'error','message'=>'No sitecode data found');	
			}
		}
		else
		{
	...
	private function resident_addcare_limit($info)
	{
		$limit='';
		$resident_id=@trim($info['resident_id']);
		if(!empty($resident_id))
		{
			...
			if($limit>0)
			{
				$list=$this->api_model->get_my_care($resident_id);
				//$newlist=json_encode($list);
				return array('status'=>'success','noc'=>$limit,'care_list'=>$list);
			}
			else
			{
				return array('status'=>'success','noc'=>$limit);
			}
	...
	// resident -->add care (not used)
	private function check_andsend_linkcare($info)
	{

		$mobile=$info['mobile'];
		//$mobile=array('6767676767');
		$resident_id=@trim($info['resident_id']);
		//$resident_id='40';
		if(!empty($mobile) && !empty($resident_id))
		{
					$existdata=array();
					$data=array();
					if(count($mobile)>0)
					{
						...
						{
							$caremob=$mobile[$j];
							$data[]=$this->api_model->check_care_exist($caremob,$resident_id);
					
						}
						$data=array_filter($data);
						if(count($data)>0)
						{
							$resident_info=$this->api_model->get_resident_info($resident_id);
							if(!empty($resident_info))
							{
								$name=$resident_info['first_name'].' '.$resident_info['last_name'];
								$resident_mob=$resident_info['mobile'];
								  $this->load->library('plivo');
									$admin_id=$this->api_model->get_admin_id_ofresident($resident_id);
									$dataq['admin_info']=$this->api_model->get_admindata(array("admin_id" => $admin_id));
									if(!empty($admin_id))
									{
										...
										{
											 if($data[$j]['status']=='notexist')
											{

													$caremob=$data[$j]['mobile_num'];
												$site_url = base_url('index.php/applink');
												  $message="$name invited you to view their calendar at $property_name. You can review the calendar of the residents using the number $resident_mob while registering.
												  \nclick here: $site_url";
												
												$senddata = array(
												'src' => '+15125663933', // +18888209850   15125663933 The phone number to use as the caller id (with the country code). E.g. For USA 15671234567
												'dst' => '+1'.$caremob, // The number to which the message needs to be send (regular phone numbers must be prefixed with country code but without the ‘+’ sign) E.g., For USA 15677654321.
												'text' => $message, // The text to send
												'type' => 'sms', //The type of message. Should be 'sms' for a text message. Defaults to 'sms'
												'url' => base_url() . 'index.php/plivo_test/receive_sms', // The URL which will be called with the status of the message.
												'method' => 'POST', // The method used to call the URL. Defaults to. POST
												);
												$response_array = $this->plivo->send_sms($senddata);

													if ($response_array[0] == '200')
													{
														//$data["response"] = json_decode($response_array[1], TRUE);
												$query_status[]=$this->api_model->insert_temp_care($caremob,$resident_id,$admin_id);
												...
												$query_status[]=$this->api_model->insert_temp_care($caremob,$resident_id,$admin_id);
													...
													{
											         return array('query_status'=>'error','message'=>'plivo service error.','result'=>$response_array);
														//$query_status[]=array;
														/*
														* the response wasn't good, show the error
														*/
											...
											if(count($query_status)==count($mobile))
											{
											return array('query_status'=>'success','message'=>'Successfully sent invitation to all non existing care.','result'=>$data);
											}
											else
											{
											return array('query_status'=>'success','message'=>'Sent invitation to non existing numbers.','result'=>$data);
											}
									}
									else
									{
										return array('status'=>'error','message'=>'Unable to get admin details');
									}
							}
							else
							{
								return array('status'=>'error','message'=>'Resident Data not Found!!');
							}
						}
	...
	private function resend_linktocare($info)
	{
		$mobile=$info['mobile'];
		//$mobile=array('8791473506');
		$resident_id=@trim($info['resident_id']);
		//$resident_id='40';
		if(!empty($mobile) && !empty($resident_id))
		{
								  ...
								  $this->load->library('plivo');
									...
									if(!empty($admin_id))
									{
										...
										for($j=0;$j<count($mobile);$j++)
										{
												...
												$senddata = array(
												'src' => '+15125663933', // +18888209850   15125663933 The phone number to use as the caller id (with the country code). E.g. For USA 15671234567
												'dst' => '+1'.$mobile[$j], // The number to which the message needs to be send (regular phone numbers must be prefixed with country code but without the ‘+’ sign) E.g., For USA 15677654321.
												'text' => $message, // The text to send
												'type' => 'sms', //The type of message. Should be 'sms' for a text message. Defaults to 'sms'
												'url' => base_url() . 'index.php/plivo_test/receive_sms', // The URL which will be called with the status of the message.
												'method' => 'POST', // The method used to call the URL. Defaults to. POST
											...
											if(count($query_status)==count($mobile))
											{
											return array('query_status'=>'success','message'=>'Successfully sent invitation to all .');
											}
											else
											{
											return array('query_status'=>'success','message'=>'Sent invitation to all numbers.');
											}
									}
									else
									{
