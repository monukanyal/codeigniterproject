...
class User extends CI_Controller {
	...
	function __construct() 
   	{
	    ...
	    $this->load->model('dashboard_model'); //@mkcode
	...
	public function childindex($parrentId)
	{
		...
		$data_feed = array("admin_id" => $this->session->userdata['logged_in']['admin_id']);
		//echo $parrentId;
		$data['arrUserP'] = $this->user_model->getparentuser($parrentId);
	...
	public function child_listing()
	{
		...
		//$pID =  array("parrent" => $parrentId);
		$data['arrUser'] = $this->user_model->allchilduser($admin_id);
		$data['approvalcare']=$this->user_model->get_approval_care($admin_id);
		$data["assets_path"] = base_url().$this->config->item('assets_path');
		$this->load->view('includes/partials/header',$data);
		$this->load->view('includes/partials/sidebar',$data);
		$this->load->view('includes/partials/topmenu',$data);
		$this->load->view('user/child_listing',$data);
		$this->load->view('includes/partials/footer',$data);
	}
	...
	function show($userId) {		
		...
		$admin_id = $this->session->userdata['logged_in']['admin_id'];
		//---------------------27 march-----------------------------------
		$assoc_arr= $this->Event_model->get_event_id($userId,$admin_id);
		if(!empty($assoc_arr))
		{
			$data['old_events']= $this->Event_model->get_events_old($assoc_arr,$admin_id);
			$data['upcoming_events']= $this->Event_model->get_events_up($assoc_arr,$admin_id);
			
		}
	
		//---------------------28 march------------------------------------
		$data['old_planmeals']= $this->Meal_model->get_planmeal_old($userId,$admin_id);
		$data['up_planmeals']= $this->Meal_model->get_planmeal_up($userId,$admin_id);
		
		$data['user_info']=$this->user_model->get_user(array("id" => $userId, "admin_id" => $admin_id));
		//----@mkcode---
		if($data['user_info']['user_type']=='child')
		{
			$data['care_belongs_to']=  $this->user_model->get_care_belongs_resident($userId,$admin_id);        
			$data['care_subscription']=$this->Event_model->check_care_subscription($userId);
			$data['only_care_pay']=$this->Event_model->check_status_onlycarepay($admin_id);
		}
		else
		{
			$data['arrUserP'] = $this->user_model->getparentuser($userId);
			$pID =  array("parrent" => $userId);
			$data['arrUser'] = $this->user_model->childuser($pID);
		}
		//--------------
		//print_r($data['care_subscription']);
	...
	*************************/
	function _generateform($default_data,$validate=0,$current_url)
	{
		...
		$data['form']['mobile']['field']=form_input(array('name'=>'mobile','type'=>'text','id'=>'mobile','value'=>$default_data['mobile'],'class'=>'form-control','autofocus'=>'true','required'=>'required','minlength'=>'9', 'maxlength'=>'14','placeholder'=>'xxx-xxx-xxxx' ));
...
//----------get country name using zipcode--------
function getcountry($zipcode)
{

	$place_url="https://maps.googleapis.com/maps/api/geocode/json?address=$zipcode&key=AIzaSyB1qIbPYFTIxMjgoLDf6K4frW2By6SPu54";
		 $georesult = file_get_contents($place_url); 
	    $output= json_decode($georesult);
	    return $output->results[0]->address_components[3]->short_name;
}
	...
	function add() {
		...
		//$default_data['countrycode'] = $this->input->post('countrycode');
		//$zipcode=$this->input->post('pincode');

		/*$country_short_name=$this->getcountry($zipcode);
			if($country_short_name=='IN')
			{
			...
			//{   		
			    ...
			    if (!empty($default_data['mobile'])) {
			    	...
			    	$sms_data = array(
					    ...
					    'dst' => $countrycode.$default_data['mobile'], // The number to which the message needs to be send (regular phone numbers must be prefixed with country code but without the ‘+’ sign) E.g., For USA 15677654321.
					  ...
					  if ($response_array[0] == '200')
				        {
				            ...
				            $entry_status=$this->user_model->msg_counter_inc($admin_id);
				            if($entry_status)
				            {
	...
	function edit($userId,$start=0)
	{
		...
		if(isset($_POST['submit'])) 
		{
			...
			if($this->form_validation->run() == TRUE)				
			{
			    ...
			    if ($edit_data['user_type'] == "child") {
			    	redirect(site_url('user/child_listing'));
		...
		$form_data['user_type_db'] = $edit_data['user_type'];
	...
	function child($parentId) {
		...
		$dataq['admin_info']=$this->user_model->get_admindata(array("admin_id" => $admin_id));
		//------------------------------------------@mkcode---------------
		$limit_care=$this->user_model->check_care_account_limit($parentId);
		//----------------------------@mkcode end-------------------------
		$data['arrUser'] = $this->user_model->getparentuser($parentId);
		$default_data['property_name'] = $dataq['admin_info'][0]['address'];
		$default_data['room_no'] = $this->input->post('room_no');
		$default_data['city'] = $this->input->post('city');	
		$default_data['state'] = $this->input->post('state');	
		...
		$default_data['appcode'] = 	$rand_res;		
		$default_data['appactive'] = 0;		
		$default_data['approved']=1;  //<---@mkcode
		...
		if(isset($_POST['submit'])) 
		{
			...
			$default_data['mobile'] = $phone_number;
			$child_mobile= $child_phone_number;
			$default_data['child_mobile'] = $child_phone_number;
			//echo $phone_number;
			$data_feed['phoneno'] = $phone_number;
			...
			//{  
				...
				$default_data['is_active'] = ($this->input->post('is_active'))?(integer) $this->input->post('is_active'):0;	
       			...
       			$default_data['belongs_to'] = $belongs_to;
			    ...
			    if (!empty($default_data['mobile'])) {
			        ...
			        $site_url = base_url('index.php/applink');
			    	$sms_data = array(
					    'src' => '+15125663933', // 18888209850The phone number to use as the caller id (with the country code). E.g. For USA 15671234567
		...
		{	//----@mkcode -------------
			//echo "care limit: ".$limit_care."<br>";
			if($limit_care!=4)
			{

				$form_data = $this->_generateform($default_data,0,$self_url);
				
				$form_data['carelist']=$this->user_model->all_care_list($parentId);
				// print_r($form_data);
			}
			else
			{
				$form_data['Message']="<h4>Care Account Exceeds 4 , Not Allowed To Add New Care A/c .</h4>";
			}
		}
	...
	//@mkcode start
	public function update_belongs_another()
	{
		$careid = $this->input->post('careid');
		$residentid = $this->input->post('residentid');
		$this->user_model->update_belongs_ofcare($careid,$residentid);
		//print_r($data);
	}

	public function approve_care_now()
	{
		$care_id = $this->input->post('care_id');
		$this->user_model->approve_care_now($care_id);
	}
	//@mkcode end
	...
	public function delete_child($userId)
	{
		$admin_id = $this->session->userdata['logged_in']['admin_id'];
		$default_data['id'] = $userId;
		$default_data['admin_id'] = $admin_id;
		$default_data['is_deleted'] = 1;
		// @mkcode added one line
		$default_data['user_type']='child';
		//-----------------------------
		$id_record=$this->user_model->delete($default_data, 'ci_user');
    ...
    {
    	$zcode = $_POST['zcode'];
    	$data = $this->user_model->get_countrycode($zcode);
    	/*if(empty($data)){
    		$data['result']=false;
    	}*/
    	return $data;
