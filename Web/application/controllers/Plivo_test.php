...
class Plivo_test extends CI_Controller
{
    ...
    public function send_sms()
    {
      ...
      foreach($userids as  $userid) 
      {
        ...
        $sms_data = array(
            'src' => '+18888209850', /** Old Number : 15125663933 18888209850 15123573060 **/  //The phone number to use as the caller id (with the country code). E.g. For USA 15671234567
        ...
        if ($response_array[0] == '200')
        {
            $data["response"] = json_decode($response_array[1], TRUE);
             ...
             $entry_status=$this->user_model->msg_counter_inc_bulk($admin_id,$result_count);
            if($entry_status)
            {
...
//            'sip_headers' => ''
    ...
    public function receive_call()
    {
     ...
     public function get_messages() {
       ...
       /* if(!empty($_REQUEST))
        {*/
            // $_REQUEST['From']='17374441555';
            // $_REQUEST['Text']="This is a test message of the emergency system.Testing function (2x609) to ensure alerting is working please press send now";
            if(!empty($_REQUEST))
            {
                $response = $_REQUEST;
                ...
                $this->load->model('user_model');
                 $fn=substr($from_number,1);
                 $uuid=$message[0];
                 
                     $status=$this->user_model->update_myinfo($fn,$uuid);
                     if($status)
                     {
                        log_message('error', "From: $from_number, uuid:$uuid,Text: $messageText");
                    }
                    else
                    {
                        log_message('error', "From: $from_number, Text: $messageText");
                    }
            }
            else
            {
                log_message('error', "No from number");
            }
        /* }else{
    ...
    public function resend_sms($userId)
    {
        ...
        $resendsms_data = array(
            'src' => '+18888209850', /** Old Number :  18888209850 15123573060 15125663933 **/ //The phone number to use as the caller id (with the country code). E.g. For USA 15671234567
