<?php 
class pdfexample1 extends CI_Controller{
     ...
     function index()
	 {
		...
		$arrMeals = $this->api_model->get_allmeal($where);
		if (empty($arrMeals)) {
			$data['arrmeal'] = array('status'=>'Error','message'=>'No Record Found.');			
		}
		
		// $arrMeals = $this->api_model->get_allevent($where);
		if(!empty($arrMeals))
			//print_r($arrMeals);
        {
        	foreach($arrMeals as $arrmeal)
			{
				
				$rec_val = $arrmeal['recurring'];
	            ...
	            $meetup_date =$y.'-'.$m.'-'.$d;
				$end_date = date('Y-m-d', strtotime($meetup_date . ' +90 day'));
    			...
    			$event_name=$this->api_model->getByID($meal_id, $where ='','ci_plan_meal');
				$arrmeal['meal_type'] = $event_name['name'];

				$currentDateTime = $meetup_date ." ". $arrmeal['start_time'];
				$newDateTime = date('h:i A', strtotime($currentDateTime));
				$arrmeal['event_time'] = $newDateTime;

				// print_r($arrmeal);
				// die();
 				$today = date('Y-m-d');

				$expiry_event = $end_date;
				$d = 1;
				$m =  date('m');
				$y =  date('Y');
					...
					foreach($daterange as $key => $date)
		        	{
	        		 	...
	        		 	if ($r_dates >= $m_first_date  && $r_dates <= $m_last_date ) 
						{
		                    ...
		                    if ($r_dates == $meetup_date) {
		                    	$response[$r_dates][$meal_id] = array($arrmeal);
		                    }
