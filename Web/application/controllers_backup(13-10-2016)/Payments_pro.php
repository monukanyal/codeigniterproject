...
class Payments_pro extends CI_Controller 
{
	...
	function Set_express_checkout()
	{
		...
		$default_data['first_name'] = trim($this->input->post('first_name'));
		...
		$SECFields = array(
							...
							'returnurl' => 'http://52.88.242.243/index.php/payments_pro/Do_express_checkout_payment', 							// Required.  URL to which the customer will be returned after returning from PayPal.  2048 char max.
							'cancelurl' => 'http://52.88.242.243/index.php/payments_pro/', 							// Required.  URL to which the customer will be returned if they cancel payment on PayPal's site.
			...
			if(($email !=''  && $this->auth_model->validate_email($email) == true) || $address !='')
			{
				...
				if (empty($result) && empty($result1)) {
					...
					if($result)
					{
				      ...
				      $message = 'http://52.88.242.243/index.php/Password_rest/';
	...
	function Get_billing_agreement_customer_details($token = "")
	{
	  ...
	  public function htmlmail(){
	  	 ...
	  	 $this->load->library('email');
            ...
            $config['protocol']    = 'smtp';
            $config['smtp_host']    = '52.40.49.3';
            $config['smtp_port']    = '465';
            $config['smtp_timeout'] = '7';
            $config['smtp_user']    = 'sheenam';
            $config['smtp_pass']    = 'sheenam@2704';
            ...
            $this->email->set_newline("\r\n");  
            $this->email->from('mohit_rana@esferasoft.com', 'sender_name');
