 ...
 $this->db->select('*')->from('ci_plan_mealmeta')->where('user_id',$user_id)->where('plan_meal_id',$mealid)->where("range_date","$r_dates");
    ...
    function get_allevent($data)
    {
      ...
      //  $query = $this->db->query("select * from ci_plan_event where(meetup_date BETWEEN NOW() - INTERVAL 30 DAY AND NOW()) and (admin_id='$admin_id' and is_active='1') ");  //<--CHANGED  LAST 30+ LATEST
        $query = $this->db->query("select * from ci_plan_event where(end_date >=NOW()) and (admin_id='$admin_id' and is_active='1')  ORDER BY meetup_time asc ");  
        //$this->db->select()->from('ci_plan_event')->where($where)->order_by("meetup_time","asc");
       // $query=$this->db->get();
        $len1=$query->num_rows();
        if($len1>0)
        ...
        $query2=$this->db->query("select * from ci_plan_event where((meetup_date >NOW()) and (admin_id='$admin_id' and is_active='1'))  ORDER BY meetup_time asc");
...
//----------mk----
    ...
    */
    ...
    function getsitecode_mob($mobilenum)
    {
        $this->db->select('*')->from('ci_user')->where('mobile','$mobilenum');
             $query_check=$this->db->get();
            if($query_check->num_rows()==1)
            {
                     $res=$query_check->result_array(); 
                    $admin_id=$res[0]['admin_id'];
                    $this->db->select('*')->from('ci_admin')->where('id',$admin_id);
                      $query2=$this->db->get();
                      $response=$query2->result_array();
                    return $response[0]['sitecode'];
            }
            else
            {
                return "";
