	...
	*********************************************************************************/	
	...
	function userLogin($data){
		...
		if(!empty($arrUser)){
			...
			if( $requiredData['userLogin'] == 'resident'){
				$response = array('user_id'=>$arrUser[0]['id'],'org_id'=>$arrUser[0]['admin_id'],'username'=>$arrUser[0]['first_name']." ".$arrUser[0]['last_name'],'mobile'=>$requiredData['mobilenum'],'status'=>'Success','message'=>'User Login Successfully' , 'user' => $loggedInUser );						
				return $response;
			}
			else
			{
				$response = array('user_id'=>$arrUser[0]['belongs_to'],'org_id'=>$arrUser[0]['admin_id'],'username'=>$arrUser[0]['first_name']." ".$arrUser[0]['last_name'],'mobile'=>$requiredData['mobilenum'],'status'=>'Success','message'=>'User Login Successfully' , 'user' => $loggedInUser );						
				return $response;
			}
			//mail('ekta_vashisht@esferasoft.com', "$subject", $response);
		}
