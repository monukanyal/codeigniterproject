(function(){
  ...
  module.controller('MainController', function($scope,$data) {
    ...
    // console.log(uniqueid);'1xCDE'
      ...
      $scope.manual_login = function(phonenum){
        ...
        $data.getUserLogin(phone).then(function(res) { 
      ...
      // if($scope.loginform.$valid.maxlength) {
      //   $scope.manual_login(phone);
      // }
    ...
    // $scope.showMydays();
      ...
      $scope.event_detail = function(event,date) {
        ...
        $data.selectedEventdates(eventid,date).then(function(res){
          ...
          console.log("Status :"+$scope.event_status);
 ...
 module.controller('MobileController', function($scope,$data) {
  ...
  module.controller('DoSomethingController', function($scope, $data) {
      ...
      $data.listUserdays(currentuser).then(function(res) {
          ...
          angular.forEach($scope.userevents,function(allmyeventdays) {
              /*console.log("Test All Events :"+JSON.stringify(allmyeventdays));
              var selectevents = JSON.stringify(allmyeventdays);
              var status = selectevents.status;
              console.log("Status :"+status);*/
              
              angular.forEach(allmyeventdays,function(allmylist) {
                $scope.mydaylist = allmylist;
                // console.log($scope.mydaylist);
                angular.forEach(allmylist,function(mylist){
                  $scope.allmyevents.push(mylist.name+"-"+mylist.range_date);
      ...
      $data.listUserStatusdays(currentuser).then(function(res) {
        console.log("Responses listUserStatusdays :"+JSON.stringify(res));  
          $scope.allmyevents = [];
          $scope.userevents = res[0].events;
          var object = {};
          ...
          angular.forEach($scope.userevents,function(allmyeventdays) {
              ...
              angular.forEach(allmyeventdays,function(allmylist) {
                var userStatuslist = allmylist;

                 console.log($scope.mydaylist);
                    $scope.event_status = userStatuslist.status;
                    console.log("Status :"+userStatuslist.status);
                    var status = $scope.event_status;
                    if(status == 1)
                    {
                      $scope.join_status = true;
  ...
  module.factory('$data',function($http) {
      ...
      // var apiUrl = 'http://52.40.49.3/index.php/api';
      ...
      data.getUserLogin = function(mobilenum){
         var getUsers = $http.get(apiUrl +'?func=userLogin&sec_token=YHDgjy9Q7yuDFgTE&mobilenum='+mobilenum).then(function(response){
      ...
      data.listUserStatusdays = function(currentuser) {
        var myday = $http.get(apiUrl + '?func=listUserStatusdays&sec_token=YHDgjy9Q7yuDFgTE&user_id='+currentuser.userId).then(function(response){
          // console.log(response.data);
          return response.data;
        });
        return myday;
      ...
      data.listmydays = function(currentuser) {
          var myday =  $http.get(apiUrl + '?func=listUserdays&sec_token=YHDgjy9Q7yuDFgTE&user_id='+currentuser.userId).then(function(resp) {
